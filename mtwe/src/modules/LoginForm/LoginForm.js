...
import url from 'assets/images/mtweurl.png'
...
class LoginForm extends Component {
  ...
  auth() {
    ...
    const getUserData = () => {
      if (storage.get().gettingUserData) {
        return
      }
      storage.get().set({ gettingUserData: true })
      ...
      return new Promise((resolve, reject) => {
        api.collect([
          ...
          storage.get().set({ gettingUserData: false })
          res.netGasPrice = BigNumber(res.gasPrice).div('1000000000').toString()
          res.rate = BigNumber(res.balanceToken).times('10000').div(res.totalSupply).toFixed(0, BigNumber.ROUND_DOWN)
          res.canMine = BigNumber(res.rate).gte('1')
          res.minBalance = BigNumber(res.totalSupply).div('10000').toFixed(4, BigNumber.ROUND_UP)
          // res.blockNumber = BigNumber(res.blockNumber).toNumber()
          let k = BigNumber(res.rate).toFixed(0) >> 0
          if (k > 19) {
            k = 19
          }
          k = Math.pow(2, k) >> 0
          k = 5000 / k >> 0
          res.ownMaximumReward = (50 - k / 100).toFixed(4)
          ...
          k = 5000000 / k >> 0
          */
          res.updater = storage.get().loggingOut ? null : setTimeout(() => getUserData(), 10000)
          res.loggingOut = false
          resolve()
          storage.get().set(res)
        ...
        .catch((e) => {
          storage.get().set({ gettingUserData: false })
          storage.get().set({ updater: storage.get().loggingOut ? null : setTimeout(() => getUserData(), 60000), loggingOut: false })
