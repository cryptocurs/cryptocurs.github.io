...
class Wallet extends Component {
  ...
  render() {
    ...
    const { address, netGasPrice, totalSupply, minted, balanceEther, balanceToken, rate, maximumReward, ownMaximumReward, canMine } = storage.get()
        ...
        <Row>
          <Col xs={12}>
            <div className='pull-right'>
              ...
              <i>Minted: {minted} {storage.get().tokenName}</i><br />
              <i>Total: {totalSupply} {storage.get().tokenName}</i><br />
              <Button bsSize='large' bsStyle='danger' onClick={() => {
                clearTimeout(storage.get().updater)
                storage.get().set({ address: null })
              }}>Logout</Button>
            </div>
            <div>Your Ethereum and {storage.get().tokenName} address:</div>
            <div><b>{address.addr}</b></div>
            <div>
              <Button bsSize='large' bsStyle='success' disabled={!canMine} onClick={() => this.mine()}>MINE {storage.get().tokenName} (0.0007 ETH + gas)</Button>
            </div>
            <div><b>Highest possible reward: {maximumReward} {storage.get().tokenName}</b></div>
            <div><b>Your maximum reward: {ownMaximumReward} {storage.get().tokenName}</b></div>
            <div><b>(You must have at least 0.1% of all tokens to get chance for successful mining)</b></div>
            <div style={{marginTop: 5, float: 'left'}}>
              <Switch onClick={this.autoMining.bind(this)} on={this.state.autoMining} />
            </div>
            <b>Auto-mining</b>{' '}
